...
import { getCategory, getProductsByCategory, getCategorySlugs } from "@/lib/data";
    ...
    <>
          ...
          <div className="mt-6 grid md:grid-cols-2 gap-8 items-center">
            ...
            {cat.heroImage && (
              <div className="relative h-48 rounded-lg overflow-hidden hidden md:block">
                <Image src={cat.heroImage} alt={cat.name} fill className="object-cover" sizes="50vw" />
              </div>
            )}
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        {cat.subcategories.length > 0 ? (
          ...
          /* No subcategories — show all products flat */
          <SectionReveal>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {catProducts.length === 0 && (
                <p className="text-sm text-ink/40 col-span-3">No products in this category yet. <Link href="/contact" className="text-safety-red">Contact us</Link>.</p>
              )}
            </div>
          </SectionReveal>
        )}
