...
-- SUBCATEGORIES
INSERT INTO subcategories (slug, category_id, name, description, sort_order) VALUES
('wall-cabinets',         (SELECT id FROM categories WHERE slug='first-aid-cabinets-kits'),       'Wall-Mount Cabinets',          'Lockable steel cabinets for fixed workplace locations.',              1),
('portable-kits',         (SELECT id FROM categories WHERE slug='first-aid-cabinets-kits'),       'Portable First Aid Kits',      'Compact kits for field crews, vehicles, and remote worksites.',       2),
('industrial-cabinets',   (SELECT id FROM categories WHERE slug='first-aid-cabinets-kits'),       'Industrial Cabinets',          'Heavy-duty cabinets built for manufacturing and warehouse environments.',3),
('bandages-dressings',    (SELECT id FROM categories WHERE slug='first-aid-restocking-supplies'), 'Bandages & Dressings',         'Adhesive bandages, gauze, and wound-care essentials.',                1),
('medications-ointments', (SELECT id FROM categories WHERE slug='first-aid-restocking-supplies'), 'Medications & Ointments',      'OTC medications, antiseptics, and topical treatments.',               2),
('burn-care',             (SELECT id FROM categories WHERE slug='first-aid-restocking-supplies'), 'Burn Care',                    'Burn gels, dressings, and cooling products.',                         3),
('gloves-hand-protection',(SELECT id FROM categories WHERE slug='ppe'),                           'Gloves & Hand Protection',     'Nitrile, latex, and cut-resistant gloves.',                           1),
('eye-face-protection',   (SELECT id FROM categories WHERE slug='ppe'),                           'Eye & Face Protection',        'Safety glasses, goggles, and face shields.',                          2),
('respiratory-protection',(SELECT id FROM categories WHERE slug='ppe'),                           'Respiratory Protection',       'Disposable masks, half-face respirators, and filters.',               3),
('plumbed-stations',      (SELECT id FROM categories WHERE slug='eye-wash-stations'),             'Plumbed Eyewash Stations',     'Permanent plumbed units for high-hazard areas.',                      1),
('portable-stations',     (SELECT id FROM categories WHERE slug='eye-wash-stations'),             'Portable Eyewash Stations',    'Self-contained portable units for areas without plumbing.',           2),
('eyewash-supplies',      (SELECT id FROM categories WHERE slug='eye-wash-stations'),             'Eyewash Solution & Supplies',  'Replacement saline solution, dust covers, and accessories.',          3),
('aed-units',             (SELECT id FROM categories WHERE slug='aed-supplies'),                  'AED Units',                    'FDA-cleared AEDs from leading manufacturers.',                        1),
('aed-accessories',       (SELECT id FROM categories WHERE slug='aed-supplies'),                  'AED Accessories',              'Replacement pads, batteries, cabinets, and signage.',                 2),
('aed-program-management',(SELECT id FROM categories WHERE slug='aed-supplies'),                  'AED Program Management',       'Inspection, compliance tracking, and maintenance services.',          3),
('72-hour-kits',          (SELECT id FROM categories WHERE slug='disaster-survival-kits'),        '72-Hour Emergency Kits',       'Individual and group kits with food, water, and first aid.',          1),
('office-kits',           (SELECT id FROM categories WHERE slug='disaster-survival-kits'),        'Office Emergency Kits',        'Pre-built kits sized for teams of 10, 25, 50, or 100.',              2),
('earthquake-supplies',   (SELECT id FROM categories WHERE slug='disaster-survival-kits'),        'Earthquake Preparedness Supplies','Specialty supplies for seismic-risk environments.',                 3)
